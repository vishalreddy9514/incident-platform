...
public class UserService {
  ...
  /**
   * ADMIN sees everyone (FR-19); ENGINEER can optionally filter to a single role (e.g. {@code
   * role=ENGINEER}) — this is how the incident assignment picker finds valid assignees, since an
   * ENGINEER has no other way to discover user ids to assign to. The role gate itself (who may call
   * this at all) is applied at the controller, per ADR-0002.
   */
   ...
   * accidentally locking every admin out of the system structurally impossible, without needing to
   * compute "is this the last remaining admin" on every request.
   */
  @Transactional
  public UserResponse updateRole(Long targetUserId, RoleUpdateRequest request, Long actingUserId) {
    ...
    String metadata =
        "{\"previousRole\":\"%s\",\"newRole\":\"%s\"}"
            .formatted(previousRole, request.role().name());
    auditLogRepository.save(
        new AuditLog(actor, "USER_ROLE_CHANGED", "User", target.getId(), metadata));
